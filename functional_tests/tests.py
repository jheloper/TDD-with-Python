...
class NewVisitorTest(LiveServerTestCase):
    ...
    def test_can_start_a_list_and_retrieve_it_later(self):
        ...
        # 엔터키를 누르면 새로운 URL로 바뀐다. 작업 목록에 "1: 공작깃털 사기" 아이템이 추가된다.
        inputbox.send_keys(Keys.ENTER)
        edith_list_url = self.browser.current_url
        self.assertRegex(edith_list_url, '/lists/.+')
        self.check_for_row_in_list_table('1: 공작깃털 사기')

        # 추가 아이템을 입력할 수 있는 여분의 텍스트 상자가 존재한다. 다시 "공작깃털을 이용해서 그물 만들기"라고 입력한다.
        inputbox = self.browser.find_element_by_id('id_new_item')
        ...
        # 세로운 사용자인 프란시스가 사이트에 접속한다.

        # 새로운 브라우저 세션을 이용해서 에디스의 정보가 쿠키를 통해 유입되는 것을 방지한다.
        self.browser.quit()
        self.browser = webdriver.Chrome('./chromedriver')

        # 프란시스가 홈페이지에 접속한다. 에디스의 리스트는 보이지 않는다.
        self.browser.get(self.live_server_url)
